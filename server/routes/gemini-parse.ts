...
export const handleGeminiParse: RequestHandler = async (req, res) => {
  try {
    ...
    const instruction = [
      ...
      "- Use ISO date format yyyy-MM-dd. If the date is in Jalali/Shamsi (e.g., 1403/01/02), convert to Gregorian. if date in (فروردين – ارييبهشت – خرداد – تير – مرداد – شهريور – مهر – آبان – آذر – دي – بهمن – اسفند) convert from shamsi to Gregorian (if no year present apply current year)  ",
